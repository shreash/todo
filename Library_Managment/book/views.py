...
def update(request,tid):
    if (request.method=='POST'):
        ...
        author_name_=request.POST['author_name']
        price_=request.POST['price']
        pages_=request.POST['pages']
        publication_date_=request.POST['publication_date']
        record_to_be_updated=BookTable.CustomManager.filter(id=tid)
        record_to_be_updated.update(book_name=book_name_ , author_name=author_name_ , price=price_ , pages=pages_ , publication_date= publication_date_ )
        return redirect('/')
    else:
        content={}
